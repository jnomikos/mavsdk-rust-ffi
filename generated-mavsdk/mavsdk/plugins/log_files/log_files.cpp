...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/master/protos/log_files/log_files.proto)
...
LogFiles::LogFiles(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<LogFilesImpl>(system)} {}
...
std::pair<LogFiles::Result, std::vector<LogFiles::Entry>>
LogFiles::get_entries() const {
...
bool operator==(const LogFiles::ProgressData &lhs,
...
bool operator==(const LogFiles::Entry &lhs, const LogFiles::Entry &rhs) {
  return (rhs.id == lhs.id) && (rhs.date == lhs.date) &&
