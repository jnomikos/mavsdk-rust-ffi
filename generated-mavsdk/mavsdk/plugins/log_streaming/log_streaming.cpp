...
LogStreaming::LogStreaming(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<LogStreamingImpl>(system)} {}
...
void LogStreaming::unsubscribe_log_streaming_raw(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<LogStreamingRawHandle *>(handle_ptr);
  _impl->unsubscribe_log_streaming_raw(*handle);
  delete handle;
}

LogStreaming::LogStreamingRawHandle LogStreaming::subscribe_log_streaming_raw(
...
bool operator==(const LogStreaming::LogStreamingRaw &lhs,
