...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/main/protos/follow_me/follow_me.proto)
...
public:
   ...
   */
  struct Config { /**
                   * @brief Altitude mode to configure which altitude the follow
                   * me will assume the target to be at.
                   */
    enum class FollowAltitudeMode {
      Constant,  /**< @brief Target assumed to be mobing at a constant altitude
                    of home position (where the vehicle armed). */
      Terrain,   /**< @brief Target assumed to be at the terrain level sensed by
                    the distance sensor. */
      TargetGps, /**< @brief Target GPS altitude taken into account to do 3D
                    tracking. */
    };

    /**
     * @brief Stream operator to print information about a
     * `FollowMe::FollowAltitudeMode`.
     *
   ...
   */
  friend bool operator==(const FollowMe::Config &lhs,
                         const FollowMe::Config &rhs);
   ...
   * @return `true` if items are equal.
   */
  friend bool operator==(const FollowMe::TargetLocation &lhs,
