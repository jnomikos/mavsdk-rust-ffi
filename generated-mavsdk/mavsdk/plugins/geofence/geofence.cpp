...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/master/protos/geofence/geofence.proto)
...
Geofence::Geofence(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<GeofenceImpl>(system)} {}

Geofence::~Geofence() {}

void Geofence::upload_geofence_async(GeofenceData geofence_data,
...
bool operator==(const Geofence::GeofenceData &lhs,
...
std::ostream &operator<<(std::ostream &str,
  ...
  str << "geofence_data:" << '\n' << "{\n";
  str << "    polygons: [";
  for (auto it = geofence_data.polygons.begin();
       it != geofence_data.polygons.end(); ++it) {
    str << *it;
    str << (it + 1 != geofence_data.polygons.end() ? ", " : "]\n");
  }
  str << "    circles: [";
  for (auto it = geofence_data.circles.begin();
       it != geofence_data.circles.end(); ++it) {
    str << *it;
