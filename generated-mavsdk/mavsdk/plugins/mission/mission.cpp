...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/master/protos/mission/mission.proto)
...
Mission::Mission(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<MissionImpl>(system)} {}

Mission::~Mission() {}

void Mission::upload_mission_async(MissionPlan mission_plan,
...
void Mission::set_current_mission_item_async(int32_t index,
                                             const ResultCallback callback) {
...
void Mission::unsubscribe_mission_progress(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
...
std::pair<Mission::Result, bool>
Mission::get_return_to_launch_after_mission() const {
...
operator<<(std::ostream &str,
           Mission::MissionItem::CameraAction const &camera_action) {
...
bool operator==(const Mission::MissionItem &lhs,
  ...
  return ((std::isnan(rhs.latitude_deg) && std::isnan(lhs.latitude_deg)) ||
         ...
         ((std::isnan(rhs.camera_photo_interval_s) &&
           std::isnan(lhs.camera_photo_interval_s)) ||
          rhs.camera_photo_interval_s == lhs.camera_photo_interval_s) &&
         ((std::isnan(rhs.acceptance_radius_m) &&
           std::isnan(lhs.acceptance_radius_m)) ||
          rhs.acceptance_radius_m == lhs.acceptance_radius_m) &&
         ((std::isnan(rhs.yaw_deg) && std::isnan(lhs.yaw_deg)) ||
          rhs.yaw_deg == lhs.yaw_deg) &&
         ((std::isnan(rhs.camera_photo_distance_m) &&
           std::isnan(lhs.camera_photo_distance_m)) ||
...
bool operator==(const Mission::MissionPlan &lhs,
...
bool operator==(const Mission::MissionProgress &lhs,
...
bool operator==(const Mission::ProgressData &lhs,
                const Mission::ProgressData &rhs) {
  return ((std::isnan(rhs.progress) && std::isnan(lhs.progress)) ||
...
bool operator==(const Mission::ProgressDataOrMission &lhs,
