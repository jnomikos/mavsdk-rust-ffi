...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/master/protos/info/info.proto)
...
Info::Info(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<InfoImpl>(system)} {}
...
void Info::unsubscribe_flight_information(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<FlightInformationHandle *>(handle_ptr);
...
std::ostream &operator<<(std::ostream &str,
  ...
  str << "    duration_since_arming_ms: "
      << flight_info.duration_since_arming_ms << '\n';
  str << "    duration_since_takeoff_ms: "
      << flight_info.duration_since_takeoff_ms << '\n';
  str << '}';
  return str;
}

bool operator==(const Info::Identification &lhs,
...
bool operator==(const Info::Product &lhs, const Info::Product &rhs) {
  return (rhs.vendor_id == lhs.vendor_id) &&
...
std::ostream &operator<<(std::ostream &str,
                         Info::Version::FlightSoftwareVersionType const
