...
ServerUtility::ServerUtility(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<ServerUtilityImpl>(system)} {}

ServerUtility::~ServerUtility() {}

ServerUtility::Result ServerUtility::send_status_text(StatusTextType type,
                                                      std::string text) const {
  return _impl->send_status_text(type, text);
}

std::ostream &operator<<(std::ostream &str,
                         ServerUtility::Result const &result) {
  switch (result) {
...
operator<<(std::ostream &str,
           ServerUtility::StatusTextType const &status_text_type) {
