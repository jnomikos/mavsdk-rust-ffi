...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/master/protos/transponder/transponder.proto)
...
Transponder::Transponder(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<TransponderImpl>(system)} {}
...
void Transponder::unsubscribe_transponder(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<TransponderHandle *>(handle_ptr);
...
void Transponder::set_rate_transponder_async(double rate_hz,
...
bool operator==(const Transponder::AdsbVehicle &lhs,
  ...
  return (rhs.icao_address == lhs.icao_address) &&
         ...
         ((std::isnan(rhs.horizontal_velocity_m_s) &&
           std::isnan(lhs.horizontal_velocity_m_s)) ||
          rhs.horizontal_velocity_m_s == lhs.horizontal_velocity_m_s) &&
         ((std::isnan(rhs.vertical_velocity_m_s) &&
           std::isnan(lhs.vertical_velocity_m_s)) ||
          rhs.vertical_velocity_m_s == lhs.vertical_velocity_m_s) &&
         (rhs.callsign == lhs.callsign) &&
         (rhs.emitter_type == lhs.emitter_type) && (rhs.squawk == lhs.squawk) &&
