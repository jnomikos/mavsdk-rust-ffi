...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/main/protos/offboard/offboard.proto)
 ...
 *
 * The module is called offboard because the commands can be sent from external
 * sources as opposed to onboard control right inside the autopilot "board".
 *
...
public:
  ...
  /**
   * @brief Type for attitude body angles in NED reference frame (roll, pitch,
   * yaw and thrust)
   */
  struct Attitude {
   ...
   */
  friend bool operator==(const Offboard::Attitude &lhs,
                         const Offboard::Attitude &rhs);
  ...
  friend std::ostream &operator<<(std::ostream &str,
                                  Offboard::Attitude const &attitude);

  /**
   * @brief Eight controls that will be given to the group. Each control is a
   * normalized
  ...
  /**
   * @brief Equal operator to compare two `Offboard::ActuatorControlGroup`
   * objects.
   *
   * @return `true` if items are equal.
   */
  friend bool operator==(const Offboard::ActuatorControlGroup &lhs,
                         const Offboard::ActuatorControlGroup &rhs);
  ...
  operator<<(std::ostream &str,
             Offboard::ActuatorControlGroup const &actuator_control_group);

  /**
   * @brief Type for actuator control.
   *
   * Control members should be normed to -1..+1 where 0 is neutral position.
   * Throttle for single rotation direction motors is 0..1, negative range for
   * reverse direction.
   *
   * One group support eight controls.
   *
   * Up to 16 actuator controls can be set. To ignore an output group, set all
   * it controls to NaN. If one or more controls in group is not NaN, then all
   * NaN controls will sent as zero. The first 8 actuator controls internally
   * map to control group 0, the latter 8 actuator controls map to control
   * group 1. Depending on what controls are set (instead of NaN) 1 or 2 MAVLink
   ...
   * @return `true` if items are equal.
   */
  friend bool operator==(const Offboard::ActuatorControl &lhs,
                         const Offboard::ActuatorControl &rhs);
  ...
  friend std::ostream &
  operator<<(std::ostream &str,
             Offboard::ActuatorControl const &actuator_control);

  /**
   * @brief Type for attitude rate commands in body coordinates (roll, pitch,
   * yaw angular rate and thrust)
   */
  struct AttitudeRate {
   ...
   * @return `true` if items are equal.
   */
  friend bool operator==(const Offboard::AttitudeRate &lhs,
                         const Offboard::AttitudeRate &rhs);
   ...
   */
  friend std::ostream &operator<<(std::ostream &str,
                                  Offboard::AttitudeRate const &attitude_rate);

  /**
   * @brief Type for position commands in NED (North East Down) coordinates and
   * yaw.
   */
   ...
   * @return `true` if items are equal.
   */
  friend bool operator==(const Offboard::PositionNedYaw &lhs,
                         const Offboard::PositionNedYaw &rhs);
  ...
  friend std::ostream &
  operator<<(std::ostream &str,
             Offboard::PositionNedYaw const &position_ned_yaw);

  /**
   * @brief Type for position commands in Global (Latitude, Longitude, Altitude)
   * coordinates and yaw.
   */
   ...
   * @return `true` if items are equal.
   */
  friend bool operator==(const Offboard::PositionGlobalYaw &lhs,
                         const Offboard::PositionGlobalYaw &rhs);
  ...
  friend std::ostream &
  operator<<(std::ostream &str,
             Offboard::PositionGlobalYaw const &position_global_yaw);
  ...
  /**
   * @brief Equal operator to compare two `Offboard::VelocityBodyYawspeed`
   * objects.
   *
   * @return `true` if items are equal.
   */
  friend bool operator==(const Offboard::VelocityBodyYawspeed &lhs,
  ...
  /**
   * @brief Type for velocity commands in NED (North East Down) coordinates and
   * yaw.
   */
   ...
   * @return `true` if items are equal.
   */
  friend bool operator==(const Offboard::VelocityNedYaw &lhs,
                         const Offboard::VelocityNedYaw &rhs);
  ...
  friend std::ostream &
  operator<<(std::ostream &str,
             Offboard::VelocityNedYaw const &velocity_ned_yaw);
   ...
   * @return `true` if items are equal.
   */
  friend bool operator==(const Offboard::AccelerationNed &lhs,
                         const Offboard::AccelerationNed &rhs);
  ...
  friend std::ostream &
  operator<<(std::ostream &str,
             Offboard::AccelerationNed const &acceleration_ned);
   ...
   * @brief Start offboard control.
   *
   * This function is blocking. See 'start_async' for the non-blocking
   counterpart.
   *
   ...
   *
   * The vehicle will be put into Hold mode:
   * https://docs.px4.io/en/flight_modes/hold.html
   *
   ...
   * The vehicle will be put into Hold mode:
   https://docs.px4.io/en/flight_modes/hold.html
   *
   * This function is blocking. See 'stop_async' for the non-blocking
   counterpart.
   *
  ...
  /**
   * @brief Set the attitude in terms of roll, pitch and yaw in degrees with
   thrust.
   *
   * This function is blocking.
   *
   ...
   *
   * First 8 controls will go to control group 0, the following 8 controls to
   control group 1 (if
  ...
  /**
   * @brief Set the attitude rate in terms of pitch, roll and yaw angular rate
   along with thrust.
   *
   * This function is blocking.
   *
  ...
  /**
   * @brief Set the position in Global coordinates (latitude, longitude,
   altitude) and yaw
   *
   * This function is blocking.
   *
  ...
  /**
   * @brief Set the velocity in body coordinates and yaw angular rate. Not
   available for fixed-wing aircraft.
   *
   * This function is blocking.
   *
  ...
  /**
   * @brief Set the velocity in NED coordinates and yaw. Not available for
   fixed-wing aircraft.
   *
   * This function is blocking.
   *
  ...
  /**
   * @brief Set the position in NED coordinates, with the velocity to be used as
   feed-forward.
   *
   * This function is blocking.
   *
   ...
   */
  Result set_position_velocity_ned(PositionNedYaw position_ned_yaw,
                                   VelocityNedYaw velocity_ned_yaw) const;

  /**
   * @brief Set the position, velocity and acceleration in NED coordinates, with
   velocity and acceleration used as feed-forward.
   *
   * This function is blocking.
   *
  ...
  Result set_position_velocity_acceleration_ned(
      PositionNedYaw position_ned_yaw, VelocityNedYaw velocity_ned_yaw,
