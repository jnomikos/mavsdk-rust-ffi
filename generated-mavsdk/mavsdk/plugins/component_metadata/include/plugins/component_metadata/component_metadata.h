...
public:
  ...
  enum class MetadataType {
    AllCompleted, /**< @brief This is set in the subscription callback when all
                     metadata types completed for a given component ID. */
  ...
  /**
   * @brief Equal operator to compare two `ComponentMetadata::MetadataData`
   * objects.
   *
   * @return `true` if items are equal.
   */
  friend bool operator==(const ComponentMetadata::MetadataData &lhs,
  ...
  /**
   * @brief Equal operator to compare two `ComponentMetadata::MetadataUpdate`
   * objects.
   *
   * @return `true` if items are equal.
   */
  friend bool operator==(const ComponentMetadata::MetadataUpdate &lhs,
  ...
  /**
   * @brief Request metadata from a specific component. This is used to start
   requesting metadata from a component.
   * The metadata can later be accessed via subscription (see below) or
   GetMetadata.
   *
  ...
  /**
   * @brief Request metadata from the autopilot component. This is used to start
   requesting metadata from the autopilot.
   * The metadata can later be accessed via subscription (see below) or
   GetMetadata.
   *
   * This function is blocking.
   ...
   */
  void unsubscribe_metadata_available(uintptr_t handle_ptr);

  /**
   * @brief Register a callback that gets called when metadata is available
   */
  MetadataAvailableHandle
  subscribe_metadata_available(const MetadataAvailableCallback &callback);
  ...
  /**
   * @brief Access metadata. This can be used if you know the metadata is
   * available already, otherwise use the subscription to get notified when it
   * becomes available.
   *
   * This function is blocking.
   *
