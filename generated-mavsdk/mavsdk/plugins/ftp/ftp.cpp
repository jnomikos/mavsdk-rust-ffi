...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/master/protos/ftp/ftp.proto)
...
Ftp::Ftp(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<FtpImpl>(system)} {}
...
void Ftp::list_directory_async(std::string remote_dir,
                               const ListDirectoryCallback callback) {
  _impl->list_directory_async(remote_dir, callback);
}

std::pair<Ftp::Result, Ftp::ListDirectoryData>
Ftp::list_directory(std::string remote_dir) const {
  return _impl->list_directory(remote_dir);
}

void Ftp::create_directory_async(std::string remote_dir,
...
void Ftp::remove_directory_async(std::string remote_dir,
                                 const ResultCallback callback) {
...
void Ftp::remove_file_async(std::string remote_file_path,
                            const ResultCallback callback) {
...
void Ftp::rename_async(std::string remote_from_path, std::string remote_to_path,
                       const ResultCallback callback) {
  _impl->rename_async(remote_from_path, remote_to_path, callback);
}

Ftp::Result Ftp::rename(std::string remote_from_path,
                        std::string remote_to_path) const {
  return _impl->rename(remote_from_path, remote_to_path);
}

void Ftp::are_files_identical_async(std::string local_file_path,
...
Ftp::are_files_identical(std::string local_file_path,
...
bool operator==(const Ftp::ListDirectoryData &lhs,
...
std::ostream &operator<<(std::ostream &str,
  ...
  str << "    dirs: [";
  for (auto it = list_directory_data.dirs.begin();
       it != list_directory_data.dirs.end(); ++it) {
    str << *it;
    str << (it + 1 != list_directory_data.dirs.end() ? ", " : "]\n");
  }
  str << "    files: [";
  for (auto it = list_directory_data.files.begin();
       it != list_directory_data.files.end(); ++it) {
    str << *it;
...
bool operator==(const Ftp::ProgressData &lhs, const Ftp::ProgressData &rhs) {
  return (rhs.bytes_transferred == lhs.bytes_transferred) &&
