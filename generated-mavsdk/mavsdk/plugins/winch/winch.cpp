...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/master/protos/winch/winch.proto)
...
Winch::Winch(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<WinchImpl>(system)} {}
...
void Winch::unsubscribe_status(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<StatusHandle *>(handle_ptr);
...
Winch::Status Winch::status() const { return _impl->status(); }
...
void Winch::relative_length_control_async(uint32_t instance, float length_m,
                                          float rate_m_s,
                                          const ResultCallback callback) {
  _impl->relative_length_control_async(instance, length_m, rate_m_s, callback);
}

Winch::Result Winch::relative_length_control(uint32_t instance, float length_m,
                                             float rate_m_s) const {
  return _impl->relative_length_control(instance, length_m, rate_m_s);
}

void Winch::rate_control_async(uint32_t instance, float rate_m_s,
                               const ResultCallback callback) {
...
void Winch::abandon_line_async(uint32_t instance,
                               const ResultCallback callback) {
...
void Winch::load_payload_async(uint32_t instance,
                               const ResultCallback callback) {
...
bool operator==(const Winch::StatusFlags &lhs, const Winch::StatusFlags &rhs) {
  return (rhs.healthy == lhs.healthy) &&
         ...
         (rhs.locking == lhs.locking) && (rhs.load_line == lhs.load_line) &&
