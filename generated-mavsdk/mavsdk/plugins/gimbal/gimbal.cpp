...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/master/protos/gimbal/gimbal.proto)
...
Gimbal::Gimbal(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<GimbalImpl>(system)} {}

Gimbal::~Gimbal() {}

void Gimbal::set_angles_async(int32_t gimbal_id, float roll_deg,
                              float pitch_deg, float yaw_deg,
                              GimbalMode gimbal_mode, SendMode send_mode,
                              const ResultCallback callback) {
  _impl->set_angles_async(gimbal_id, roll_deg, pitch_deg, yaw_deg, gimbal_mode,
                          send_mode, callback);
}

Gimbal::Result Gimbal::set_angles(int32_t gimbal_id, float roll_deg,
                                  float pitch_deg, float yaw_deg,
                                  GimbalMode gimbal_mode,
                                  SendMode send_mode) const {
  return _impl->set_angles(gimbal_id, roll_deg, pitch_deg, yaw_deg, gimbal_mode,
                           send_mode);
}

void Gimbal::set_angular_rates_async(int32_t gimbal_id, float roll_rate_deg_s,
                                     float pitch_rate_deg_s,
                                     float yaw_rate_deg_s,
                                     GimbalMode gimbal_mode, SendMode send_mode,
                                     const ResultCallback callback) {
  _impl->set_angular_rates_async(gimbal_id, roll_rate_deg_s, pitch_rate_deg_s,
                                 yaw_rate_deg_s, gimbal_mode, send_mode,
                                 callback);
}

Gimbal::Result
Gimbal::set_angular_rates(int32_t gimbal_id, float roll_rate_deg_s,
                          float pitch_rate_deg_s, float yaw_rate_deg_s,
                          GimbalMode gimbal_mode, SendMode send_mode) const {
  return _impl->set_angular_rates(gimbal_id, roll_rate_deg_s, pitch_rate_deg_s,
                                  yaw_rate_deg_s, gimbal_mode, send_mode);
}

void Gimbal::set_roi_location_async(int32_t gimbal_id, double latitude_deg,
                                    double longitude_deg, float altitude_m,
                                    const ResultCallback callback) {
  _impl->set_roi_location_async(gimbal_id, latitude_deg, longitude_deg,
                                altitude_m, callback);
}

Gimbal::Result Gimbal::set_roi_location(int32_t gimbal_id, double latitude_deg,
                                        double longitude_deg,
                                        float altitude_m) const {
  return _impl->set_roi_location(gimbal_id, latitude_deg, longitude_deg,
                                 altitude_m);
}

void Gimbal::take_control_async(int32_t gimbal_id, ControlMode control_mode,
                                const ResultCallback callback) {
  _impl->take_control_async(gimbal_id, control_mode, callback);
}

Gimbal::Result Gimbal::take_control(int32_t gimbal_id,
                                    ControlMode control_mode) const {
  return _impl->take_control(gimbal_id, control_mode);
}

void Gimbal::release_control_async(int32_t gimbal_id,
                                   const ResultCallback callback) {
...
void Gimbal::unsubscribe_gimbal_list(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<GimbalListHandle *>(handle_ptr);
  _impl->unsubscribe_gimbal_list(*handle);
  delete handle;
}

Gimbal::GimbalListHandle
Gimbal::subscribe_gimbal_list(const GimbalListCallback &callback) {
...
void Gimbal::unsubscribe_control_status(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<ControlStatusHandle *>(handle_ptr);
  _impl->unsubscribe_control_status(*handle);
  delete handle;
}

Gimbal::ControlStatusHandle
Gimbal::subscribe_control_status(const ControlStatusCallback &callback) {
...
std::pair<Gimbal::Result, Gimbal::ControlStatus>
Gimbal::get_control_status(int32_t gimbal_id) const {
...
void Gimbal::unsubscribe_attitude(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<AttitudeHandle *>(handle_ptr);
  _impl->unsubscribe_attitude(*handle);
  delete handle;
}

Gimbal::AttitudeHandle
Gimbal::subscribe_attitude(const AttitudeCallback &callback) {
...
std::pair<Gimbal::Result, Gimbal::Attitude>
Gimbal::get_attitude(int32_t gimbal_id) const {
...
bool operator==(const Gimbal::AngularVelocityBody &lhs,
...
std::ostream &operator<<(std::ostream &str,
  ...
  str << "    gimbal_manager_component_id: "
      << gimbal_item.gimbal_manager_component_id << '\n';
...
std::ostream &operator<<(std::ostream &str,
  ...
  str << "gimbal_list:" << '\n' << "{\n";
  str << "    gimbals: [";
  for (auto it = gimbal_list.gimbals.begin(); it != gimbal_list.gimbals.end();
       ++it) {
    str << *it;
...
bool operator==(const Gimbal::ControlStatus &lhs,
...
std::ostream &operator<<(std::ostream &str,
  ...
  str << "    compid_primary_control: " << control_status.compid_primary_control
      << '\n';
  str << "    sysid_secondary_control: "
      << control_status.sysid_secondary_control << '\n';
  str << "    compid_secondary_control: "
      << control_status.compid_secondary_control << '\n';
  str << '}';
  return str;
}
