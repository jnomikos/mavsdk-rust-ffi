...
// (see
// https://github.com/mavlink/MAVSDK-Proto/blob/master/protos/events/events.proto)
...
Events::Events(std::shared_ptr<System> system)
    : PluginBase(), _impl{std::make_unique<EventsImpl>(system)} {}
...
void Events::unsubscribe_events(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<EventsHandle *>(handle_ptr);
...
void Events::unsubscribe_health_and_arming_checks(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<HealthAndArmingChecksHandle *>(handle_ptr);
  _impl->unsubscribe_health_and_arming_checks(*handle);
  delete handle;
}

Events::HealthAndArmingChecksHandle Events::subscribe_health_and_arming_checks(
    const HealthAndArmingChecksCallback &callback) {
  return _impl->subscribe_health_and_arming_checks(callback);
}

void Events::unsubscribe_health_and_arming_checks(
...
bool operator==(const Events::HealthAndArmingCheckProblem &lhs,
...
std::ostream &operator<<(std::ostream &str,
                         Events::HealthAndArmingCheckProblem const
                             &health_and_arming_check_problem) {
  str << std::setprecision(15);
  ...
  str << "    health_component: "
      << health_and_arming_check_problem.health_component << '\n';
  str << '}';
  return str;
}

bool operator==(const Events::HealthAndArmingCheckMode &lhs,
                const Events::HealthAndArmingCheckMode &rhs) {
  return (rhs.mode_name == lhs.mode_name) &&
...
std::ostream &operator<<(
    std::ostream &str,
    Events::HealthAndArmingCheckMode const &health_and_arming_check_mode) {
  str << std::setprecision(15);
  ...
  for (auto it = health_and_arming_check_mode.problems.begin();
       it != health_and_arming_check_mode.problems.end(); ++it) {
    str << *it;
...
bool operator==(const Events::HealthComponentReport &lhs,
                const Events::HealthComponentReport &rhs) {
...
operator<<(std::ostream &str,
           Events::HealthComponentReport const &health_component_report) {
  str << std::setprecision(15);
...
bool operator==(const Events::HealthAndArmingCheckReport &lhs,
...
std::ostream &operator<<(
  ...
  for (auto it = health_and_arming_check_report.health_components.begin();
       it != health_and_arming_check_report.health_components.end(); ++it) {
    str << *it;
  ...
  for (auto it = health_and_arming_check_report.all_problems.begin();
       it != health_and_arming_check_report.all_problems.end(); ++it) {
    str << *it;
