...
void ArmAuthorizerServer::unsubscribe_arm_authorization(uintptr_t handle_ptr) {
  if (!handle_ptr) {
    return;
  }
  auto handle = reinterpret_cast<ArmAuthorizationHandle *>(handle_ptr);
  _impl->unsubscribe_arm_authorization(*handle);
  delete handle;
}

ArmAuthorizerServer::ArmAuthorizationHandle
ArmAuthorizerServer::subscribe_arm_authorization(
    const ArmAuthorizationCallback &callback) {
  return _impl->subscribe_arm_authorization(callback);
}

void ArmAuthorizerServer::unsubscribe_arm_authorization(
