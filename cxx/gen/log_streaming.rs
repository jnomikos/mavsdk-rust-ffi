...
impl<'a> LogStreamingRaw<'a> {
    ...
    pub fn data_base64(&self) -> String {
        self.inner.log_streaming_get_data_base64().to_string_lossy().into_owned()
    }
}
    ...
    pub struct LogStreamingClient {
        inner: Mutex<LogStreamingInner>,
                ...
                log_streaming_raw_tx: watch::Sender<std::option::Option<
        LogStreamingRawOwned>>,
    }
    ...
    impl LogStreamingClient {
        ...
        pub fn subscribe_log_streaming_raw(&self) -> watch::Receiver<std::option::Option<
        LogStreamingRawOwned>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.log_streaming_raw_handle.is_none() {
                        // Lazy initialization of C++ 'log_streaming_raw' log_streaming stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.log_streaming_raw_tx.clone())) as *mut c_void;
