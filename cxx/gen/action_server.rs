...
impl Drop for ActionServerClient {
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(handle) = inner.arm_disarm_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_arm_disarm(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.arm_disarm_user_data
                            as *mut watch::Sender<std::option::Option<ArmDisarmStatus>>,
                    );
                }
            }
            if let Some(handle) = inner.flight_mode_change_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_flight_mode_change(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.flight_mode_change_user_data
                            as *mut watch::Sender<std::option::Option<FlightModeStatus>>,
                    );
                }
            }
            if let Some(handle) = inner.takeoff_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_takeoff(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.takeoff_user_data
                            as *mut watch::Sender<std::option::Option<TakeoffStatus>>,
                    );
                }
            }
            if let Some(handle) = inner.land_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_land(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.land_user_data as *mut watch::Sender<std::option::Option<LandStatus>>,
                    );
                }
            }
            if let Some(handle) = inner.reboot_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_reboot(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.reboot_user_data
                            as *mut watch::Sender<std::option::Option<RebootStatus>>,
                    );
                }
            }
            if let Some(handle) = inner.shutdown_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_shutdown(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.shutdown_user_data
                            as *mut watch::Sender<std::option::Option<ShutdownStatus>>,
                    );
                }
            }
            if let Some(handle) = inner.terminate_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_terminate(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.terminate_user_data
                            as *mut watch::Sender<std::option::Option<TerminateStatus>>,
                    );
                }
            }
        }
