...
impl<'a> MissionImportData<'a> {
    ...
    pub fn mission_items(&self) -> 
            std::vec::Vec<MissionItemOwned> {
            self.inner.mission_raw_get_mission_items()
                .iter()
    ...
    pub fn geofence_items(&self) -> 
            std::vec::Vec<MissionItemOwned> {
            self.inner.mission_raw_get_geofence_items()
                .iter()
                .map(|item| MissionItem::new(item).into_owned())
                .collect()
    }
    ...
    pub fn rally_items(&self) -> 
            std::vec::Vec<MissionItemOwned> {
            self.inner.mission_raw_get_rally_items()
                .iter()
                .map(|item| MissionItem::new(item).into_owned())
                .collect()
    }
    ...
    pub struct MissionRawClient {
        ...
        MissionProgressOwned>>,
                ...
                mission_changed_tx: watch::Sender<std::option::Option<
        bool>>,
    }
    ...
    impl MissionRawClient {
        ...
        MissionProgressOwned>> {
                ...
                }pub fn subscribe_mission_changed(&self) -> watch::Receiver<std::option::Option<
        bool>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.mission_changed_handle.is_none() {
                        // Lazy initialization of C++ 'mission_changed' mission_raw stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.mission_changed_tx.clone())) as *mut c_void;
        ...
        pub extern "C" fn mission_raw_mission_changed_callback_ffi(
            ...
            if user_data.is_null() { return; }
                let send_mission_changed = 
        mission_changed.clone();
