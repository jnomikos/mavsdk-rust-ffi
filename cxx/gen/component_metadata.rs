...
impl<'a> MetadataData<'a> {
    pub fn new(inner: &'a crate::component_metadata::mavsdk::ComponentMetadata_MetadataData) -> Self {
        Self { inner }
    }
    ...
    pub fn json_metadata(&self) -> String {
        self.inner.component_metadata_get_json_metadata().to_string_lossy().into_owned()
    }
}
...
impl<'a> MetadataUpdate<'a> {
    pub fn new(inner: &'a crate::component_metadata::mavsdk::ComponentMetadata_MetadataUpdate) -> Self {
        Self { inner }
    }
    ...
    pub fn r#type(&self) -> crate::component_metadata::mavsdk::ComponentMetadata_MetadataType {self.inner.component_metadata_get_type().clone()
    }
        ///  The JSON metadata
    ...
    pub struct ComponentMetadataClient {
        inner: Mutex<ComponentMetadataInner>,
                ...
                metadata_available_tx: watch::Sender<std::option::Option<
        MetadataUpdateOwned>>,
    }

    impl ComponentMetadataClient {
        pub fn new(plugin: cxx::UniquePtr<crate::component_metadata::mavsdk::ComponentMetadata>) -> Self {
        ...
        pub fn subscribe_metadata_available(&self) -> watch::Receiver<std::option::Option<
        MetadataUpdateOwned>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.metadata_available_handle.is_none() {
                        // Lazy initialization of C++ 'metadata_available' component_metadata stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.metadata_available_tx.clone())) as *mut c_void;
