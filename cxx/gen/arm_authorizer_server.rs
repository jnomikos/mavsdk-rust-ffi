...
pub struct ArmAuthorizerServerClient {
    ...
    arm_authorization_tx: watch::Sender<std::option::Option<u32>>,
}
...
impl ArmAuthorizerServerClient {
    ...
    pub fn subscribe_arm_authorization(&self) -> watch::Receiver<std::option::Option<u32>> {
        let mut inner = self.inner.lock().unwrap();

        if inner.arm_authorization_handle.is_none() {
            // Lazy initialization of C++ 'arm_authorization' arm_authorizer_server stream.
            let user_data_ptr =
                Box::into_raw(Box::new(self.arm_authorization_tx.clone())) as *mut c_void;
...
impl Drop for ArmAuthorizerServerClient {
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(handle) = inner.arm_authorization_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_arm_authorization(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.arm_authorization_user_data
                            as *mut watch::Sender<std::option::Option<u32>>,
                    );
                }
            }
        }
...
pub extern "C" fn arm_authorizer_server_arm_authorization_callback_ffi(
    ...
    let send_arm_authorization = arm_authorization.clone();
