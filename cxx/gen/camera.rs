...
impl<'a> Setting<'a> {
    ...
    pub fn setting_description(&self) -> String {
        self.inner
            .camera_get_setting_description()
            .to_string_lossy()
            .into_owned()
    }
    ///  Selected option

    pub fn option(&self) -> OptionOwned {
        CameraOption::new(self.inner.camera_get_option()).into_owned()
    }
...
pub struct SettingOptionsOwned {
    ...
    pub setting_id: String,
    pub setting_description: String,
    pub options: std::vec::Vec<OptionOwned>,
    pub is_range: bool,
}
...
impl<'a> SettingOptions<'a> {
    ...
    pub fn options(&self) -> std::vec::Vec<OptionOwned> {
        self.inner
            .camera_get_options()
            .iter()
...
impl<'a> VideoStreamInfo<'a> {
    ...
    pub fn settings(&self) -> VideoStreamSettingsOwned {
        VideoStreamSettings::new(self.inner.camera_get_settings()).into_owned()
    }
...
impl<'a> ModeUpdate<'a> {
    ...
    pub fn mode(&self) -> crate::camera::mavsdk::Camera_Mode {
        self.inner.camera_get_mode().clone()
    }
}
...
impl<'a> VideoStreamUpdate<'a> {
    ...
    pub fn video_stream_info(&self) -> VideoStreamInfoOwned {
        VideoStreamInfo::new(self.inner.camera_get_video_stream_info()).into_owned()
    }
}
...
impl<'a> StorageUpdate<'a> {
    ...
    pub fn storage(&self) -> StorageOwned {
        Storage::new(self.inner.camera_get_storage()).into_owned()
    }
}

pub struct CurrentSettingsUpdateOwned {
    pub component_id: i32,
    pub current_settings: std::vec::Vec<SettingOwned>,
}
...
impl<'a> CurrentSettingsUpdate<'a> {
    ...
    pub fn current_settings(&self) -> std::vec::Vec<SettingOwned> {
        self.inner
            .camera_get_current_settings()
            .iter()
...
pub struct PossibleSettingOptionsUpdateOwned {
    pub component_id: i32,
    pub setting_options: std::vec::Vec<SettingOptionsOwned>,
}
...
impl<'a> PossibleSettingOptionsUpdate<'a> {
    ...
    pub fn setting_options(&self) -> std::vec::Vec<SettingOptionsOwned> {
        self.inner
            .camera_get_setting_options()
            .iter()
...
pub struct CameraListOwned {
    pub cameras: std::vec::Vec<InformationOwned>,
}
...
impl<'a> CameraList<'a> {
    ...
    pub fn cameras(&self) -> std::vec::Vec<InformationOwned> {
        self.inner
            .camera_get_cameras()
            .iter()
...
impl CameraClient {
    ...
    pub fn subscribe_camera_list(&self) -> watch::Receiver<std::option::Option<CameraListOwned>> {
    ...
    pub fn subscribe_mode(&self) -> watch::Receiver<std::option::Option<ModeUpdateOwned>> {
        let mut inner = self.inner.lock().unwrap();
    ...
    pub fn subscribe_video_stream_info(
        &self,
    ) -> watch::Receiver<std::option::Option<VideoStreamUpdateOwned>> {
        let mut inner = self.inner.lock().unwrap();

        if inner.video_stream_info_handle.is_none() {
            // Lazy initialization of C++ 'video_stream_info' camera stream.
            let user_data_ptr =
                Box::into_raw(Box::new(self.video_stream_info_tx.clone())) as *mut c_void;
    ...
    pub fn subscribe_capture_info(&self) -> watch::Receiver<std::option::Option<CaptureInfoOwned>> {
        let mut inner = self.inner.lock().unwrap();

        if inner.capture_info_handle.is_none() {
            // Lazy initialization of C++ 'capture_info' camera stream.
            let user_data_ptr =
                Box::into_raw(Box::new(self.capture_info_tx.clone())) as *mut c_void;
    ...
    pub fn subscribe_storage(&self) -> watch::Receiver<std::option::Option<StorageUpdateOwned>> {
        let mut inner = self.inner.lock().unwrap();
    ...
    pub fn subscribe_current_settings(
        &self,
    ) -> watch::Receiver<std::option::Option<CurrentSettingsUpdateOwned>> {
        let mut inner = self.inner.lock().unwrap();

        if inner.current_settings_handle.is_none() {
            // Lazy initialization of C++ 'current_settings' camera stream.
            let user_data_ptr =
                Box::into_raw(Box::new(self.current_settings_tx.clone())) as *mut c_void;
    ...
    pub fn subscribe_possible_setting_options(
        &self,
    ) -> watch::Receiver<std::option::Option<PossibleSettingOptionsUpdateOwned>> {
        let mut inner = self.inner.lock().unwrap();

        if inner.possible_setting_options_handle.is_none() {
            // Lazy initialization of C++ 'possible_setting_options' camera stream.
            let user_data_ptr =
                Box::into_raw(Box::new(self.possible_setting_options_tx.clone())) as *mut c_void;
...
impl Drop for CameraClient {
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(handle) = inner.camera_list_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_camera_list(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.camera_list_user_data
                            as *mut watch::Sender<std::option::Option<CameraListOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.mode_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_mode(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.mode_user_data
                            as *mut watch::Sender<std::option::Option<ModeUpdateOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.video_stream_info_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_video_stream_info(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.video_stream_info_user_data
                            as *mut watch::Sender<std::option::Option<VideoStreamUpdateOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.capture_info_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_capture_info(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.capture_info_user_data
                            as *mut watch::Sender<std::option::Option<CaptureInfoOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.storage_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_storage(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.storage_user_data
                            as *mut watch::Sender<std::option::Option<StorageUpdateOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.current_settings_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_current_settings(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.current_settings_user_data
                            as *mut watch::Sender<std::option::Option<CurrentSettingsUpdateOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.possible_setting_options_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_possible_setting_options(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.possible_setting_options_user_data
                            as *mut watch::Sender<
                                std::option::Option<PossibleSettingOptionsUpdateOwned>,
                            >,
                    );
                }
            }
...
pub extern "C" fn camera_camera_list_callback_ffi(
    ...
    let sender =
        unsafe { &*(user_data as *const watch::Sender<std::option::Option<CameraListOwned>>) };
...
pub extern "C" fn camera_mode_callback_ffi(
    ...
    // Cast the pointer to watch::Sender
    let sender =
        unsafe { &*(user_data as *const watch::Sender<std::option::Option<ModeUpdateOwned>>) };
...
pub extern "C" fn camera_video_stream_info_callback_ffi(
    ...
    // Cast the pointer to watch::Sender
    let sender = unsafe {
        &*(user_data as *const watch::Sender<std::option::Option<VideoStreamUpdateOwned>>)
    };
...
pub extern "C" fn camera_capture_info_callback_ffi(
    ...
    // Cast the pointer to watch::Sender
    let sender =
        unsafe { &*(user_data as *const watch::Sender<std::option::Option<CaptureInfoOwned>>) };
...
pub extern "C" fn camera_storage_callback_ffi(
    ...
    // Cast the pointer to watch::Sender
    let sender =
        unsafe { &*(user_data as *const watch::Sender<std::option::Option<StorageUpdateOwned>>) };
...
pub extern "C" fn camera_current_settings_callback_ffi(
    ...
    // Cast the pointer to watch::Sender
    let sender = unsafe {
        &*(user_data as *const watch::Sender<std::option::Option<CurrentSettingsUpdateOwned>>)
    };
...
pub extern "C" fn camera_possible_setting_options_callback_ffi(
    ...
    let send_possible_setting_options =
        PossibleSettingOptionsUpdate::new(possible_setting_options).into_owned();

    // Cast the pointer to watch::Sender
    let sender = unsafe {
        &*(user_data
            as *const watch::Sender<std::option::Option<PossibleSettingOptionsUpdateOwned>>)
    };
