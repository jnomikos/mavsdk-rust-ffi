...
impl<'a> Metadata<'a> {
    ...
    pub fn r#type(
        &self,
    ) -> crate::component_metadata_server::mavsdk::ComponentMetadataServer_MetadataType {
        self.inner.component_metadata_server_get_type().clone()
    }
    ///  The JSON metadata

    pub fn json_metadata(&self) -> String {
        self.inner
            .component_metadata_server_get_json_metadata()
            .to_string_lossy()
            .into_owned()
    }
}
...
impl ComponentMetadataServerClient {
    pub fn new(
        ...
        Self {
            inner: Mutex::new(ComponentMetadataServerInner { plugin }),
        }
    }
}
