...
impl<'a> Information<'a> {
    ...
    pub fn vendor_name(&self) -> String {
        self.inner.camera_server_get_vendor_name().to_string_lossy().into_owned()
    }
        ///  Name of the camera model

    pub fn model_name(&self) -> String {
        self.inner.camera_server_get_model_name().to_string_lossy().into_owned()
    }
        ///  Camera firmware version in major[.minor[.patch[.dev]]] format

    pub fn firmware_version(&self) -> String {
        self.inner.camera_server_get_firmware_version().to_string_lossy().into_owned()
    }
        ///  Focal length
    ...
    pub fn definition_file_uri(&self) -> String {
        self.inner.camera_server_get_definition_file_uri().to_string_lossy().into_owned()
    }
...
impl<'a> VideoStreaming<'a> {
    ...
    pub fn rtsp_uri(&self) -> String {
        self.inner.camera_server_get_rtsp_uri().to_string_lossy().into_owned()
    }
}
...
impl<'a> CaptureInfo<'a> {
    ...
    pub fn file_url(&self) -> String {
        self.inner.camera_server_get_file_url().to_string_lossy().into_owned()
    }
}
...
impl<'a> StorageInformation<'a> {
    ...
    pub fn storage_status(&self) -> crate::camera_server::mavsdk::CameraServer_StorageInformation_StorageStatus {
    ...
    pub fn storage_type(&self) -> crate::camera_server::mavsdk::CameraServer_StorageInformation_StorageType {
...
impl<'a> CaptureStatus<'a> {
    ...
    pub fn image_status(&self) -> crate::camera_server::mavsdk::CameraServer_CaptureStatus_ImageStatus {
        self.inner.camera_server_get_image_status().clone()
    }
        ///  Current status of video capturing

    pub fn video_status(&self) -> crate::camera_server::mavsdk::CameraServer_CaptureStatus_VideoStatus {
...
impl<'a> TrackRectangle<'a> {
    ...
    impl CameraServerClient {
        ...
        pub fn subscribe_take_photo(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                ...
                }pub fn subscribe_start_video(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();
                ...
                }pub fn subscribe_stop_video(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();
                ...
                }pub fn subscribe_start_video_streaming(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.start_video_streaming_handle.is_none() {
                        // Lazy initialization of C++ 'start_video_streaming' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.start_video_streaming_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_stop_video_streaming(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.stop_video_streaming_handle.is_none() {
                        // Lazy initialization of C++ 'stop_video_streaming' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.stop_video_streaming_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_set_mode(&self) -> watch::Receiver<std::option::Option<
        crate::camera_server::mavsdk::CameraServer_Mode>> {
                    let mut inner = self.inner.lock().unwrap();
                ...
                }pub fn subscribe_storage_information(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.storage_information_handle.is_none() {
                        // Lazy initialization of C++ 'storage_information' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.storage_information_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_capture_status(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.capture_status_handle.is_none() {
                        // Lazy initialization of C++ 'capture_status' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.capture_status_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_format_storage(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.format_storage_handle.is_none() {
                        // Lazy initialization of C++ 'format_storage' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.format_storage_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_reset_settings(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.reset_settings_handle.is_none() {
                        // Lazy initialization of C++ 'reset_settings' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.reset_settings_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_zoom_in_start(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.zoom_in_start_handle.is_none() {
                        // Lazy initialization of C++ 'zoom_in_start' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.zoom_in_start_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_zoom_out_start(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.zoom_out_start_handle.is_none() {
                        // Lazy initialization of C++ 'zoom_out_start' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.zoom_out_start_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_zoom_stop(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();
                ...
                }pub fn subscribe_zoom_range(&self) -> watch::Receiver<std::option::Option<
        f32>> {
                    let mut inner = self.inner.lock().unwrap();
                ...
                }pub fn subscribe_tracking_point_command(&self) -> watch::Receiver<std::option::Option<
        TrackPointOwned>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.tracking_point_command_handle.is_none() {
                        // Lazy initialization of C++ 'tracking_point_command' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.tracking_point_command_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_tracking_rectangle_command(&self) -> watch::Receiver<std::option::Option<
        TrackRectangleOwned>> {
                    let mut inner = self.inner.lock().unwrap();
                    ...
                    if inner.tracking_rectangle_command_handle.is_none() {
                        ...
                        let user_data_ptr = Box::into_raw(Box::new(self.tracking_rectangle_command_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_tracking_off_command(&self) -> watch::Receiver<std::option::Option<
        i32>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.tracking_off_command_handle.is_none() {
                        // Lazy initialization of C++ 'tracking_off_command' camera_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.tracking_off_command_tx.clone())) as *mut c_void;
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn camera_server_take_photo_callback_ffi(
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn camera_server_start_video_callback_ffi(
            user_data: *mut c_void,
            start_video: i32,
        ) {
            if user_data.is_null() { return; }
                let send_start_video = 
        start_video.clone();
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn camera_server_stop_video_callback_ffi(
            user_data: *mut c_void,
            stop_video: i32,
        ) {
            if user_data.is_null() { return; }
        ...
        pub extern "C" fn camera_server_start_video_streaming_callback_ffi(
            ...
            if user_data.is_null() { return; }
                let send_start_video_streaming = 
        start_video_streaming.clone();
        ...
        pub extern "C" fn camera_server_stop_video_streaming_callback_ffi(
            ...
            if user_data.is_null() { return; }
                let send_stop_video_streaming = 
        stop_video_streaming.clone();
        ...
        pub extern "C" fn camera_server_storage_information_callback_ffi(
            ...
            if user_data.is_null() { return; }
                let send_storage_information = 
        storage_information.clone();
        ...
        pub extern "C" fn camera_server_format_storage_callback_ffi(
            ...
            if user_data.is_null() { return; }
                let send_format_storage = 
        format_storage.clone();
        ...
        pub extern "C" fn camera_server_zoom_out_start_callback_ffi(
            ...
            if user_data.is_null() { return; }
                let send_zoom_out_start = 
        zoom_out_start.clone();
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn camera_server_zoom_stop_callback_ffi(
            user_data: *mut c_void,
            zoom_stop: i32,
        ) {
            if user_data.is_null() { return; }
        ...
        #[unsafe(no_mangle)]
        pub extern "C" fn camera_server_zoom_range_callback_ffi(
            user_data: *mut c_void,
            zoom_range: f32,
        ) {
            if user_data.is_null() { return; }
        ...
        pub extern "C" fn camera_server_tracking_rectangle_command_callback_ffi(
            ...
            if user_data.is_null() { return; }
                let send_tracking_rectangle_command = TrackRectangle::new(tracking_rectangle_command).into_owned();
        ...
        pub extern "C" fn camera_server_tracking_off_command_callback_ffi(
            ...
            if user_data.is_null() { return; }
                let send_tracking_off_command = 
        tracking_off_command.clone();
