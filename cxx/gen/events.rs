...
impl<'a> Event<'a> {
    ...
    pub fn message(&self) -> String {
        self.inner.events_get_message().to_string_lossy().into_owned()
    }
        ///  Detailed description (optional, might be multiple lines)

    pub fn description(&self) -> String {
        self.inner.events_get_description().to_string_lossy().into_owned()
    }
        ///  Log level of message

    pub fn log_level(&self) -> crate::events::mavsdk::Events_LogLevel {self.inner.events_get_log_level().clone()
    }
        ///  Namespace, e.g. "px4"

    pub fn event_namespace(&self) -> String {
        self.inner.events_get_event_namespace().to_string_lossy().into_owned()
    }
        ///  Event name (unique within the namespace)

    pub fn event_name(&self) -> String {
        self.inner.events_get_event_name().to_string_lossy().into_owned()
    }
}
...
impl<'a> HealthAndArmingCheckProblem<'a> {
    ...
    pub fn health_component(&self) -> String {
        self.inner.events_get_health_component().to_string_lossy().into_owned()
    }
}
...
pub struct HealthAndArmingCheckModeOwned {
        ...
        pub problems: 
            std::vec::Vec<HealthAndArmingCheckProblemOwned>,
}
...
impl<'a> HealthAndArmingCheckMode<'a> {
    ...
    pub fn mode_name(&self) -> String {
        self.inner.events_get_mode_name().to_string_lossy().into_owned()
    }
    ...
    pub fn problems(&self) -> 
            std::vec::Vec<HealthAndArmingCheckProblemOwned> {
            self.inner.events_get_problems()
                .iter()
...
pub struct HealthAndArmingCheckReportOwned {
        ...
        pub health_components: 
            std::vec::Vec<HealthComponentReportOwned>,
        pub all_problems: 
            std::vec::Vec<HealthAndArmingCheckProblemOwned>,
}
...
impl<'a> HealthAndArmingCheckReport<'a> {
    ...
    pub fn current_mode_intention(&self) -> HealthAndArmingCheckModeOwned {HealthAndArmingCheckMode::new(self.inner.events_get_current_mode_intention()).into_owned()
    }
        ///  Health components list (e.g. for "gps")

    pub fn health_components(&self) -> 
            std::vec::Vec<HealthComponentReportOwned> {
            self.inner.events_get_health_components()
                .iter()
    ...
    pub fn all_problems(&self) -> 
            std::vec::Vec<HealthAndArmingCheckProblemOwned> {
            self.inner.events_get_all_problems()
                .iter()
                .map(|item| HealthAndArmingCheckProblem::new(item).into_owned())
                .collect()
    }
    ...
    pub struct EventsClient {
        inner: Mutex<EventsInner>,
                ...
                events_tx: watch::Sender<std::option::Option<
        EventOwned>>,
                // Producer for the latest HealthAndArmingChecks state; broadcast to all active subscribers.
                health_and_arming_checks_tx: watch::Sender<std::option::Option<
        HealthAndArmingCheckReportOwned>>,
    }
    ...
    impl EventsClient {
        ...
        pub fn subscribe_events(&self) -> watch::Receiver<std::option::Option<
        EventOwned>> {
                ...
                }pub fn subscribe_health_and_arming_checks(&self) -> watch::Receiver<std::option::Option<
        HealthAndArmingCheckReportOwned>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.health_and_arming_checks_handle.is_none() {
                        // Lazy initialization of C++ 'health_and_arming_checks' events stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.health_and_arming_checks_tx.clone())) as *mut c_void;
        ...
        pub extern "C" fn events_health_and_arming_checks_callback_ffi(
            ...
            if user_data.is_null() { return; }
                let send_health_and_arming_checks = HealthAndArmingCheckReport::new(health_and_arming_checks).into_owned();
