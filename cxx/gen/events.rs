...
impl Drop for EventsClient {
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(handle) = inner.events_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_events(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.events_user_data
                            as *mut watch::Sender<std::option::Option<EventOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.health_and_arming_checks_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_health_and_arming_checks(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.health_and_arming_checks_user_data
                            as *mut watch::Sender<
                                std::option::Option<HealthAndArmingCheckReportOwned>,
                            >,
                    );
                }
            }
