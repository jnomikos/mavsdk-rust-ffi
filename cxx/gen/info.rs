...
impl<'a> Version<'a> {
    ...
    pub fn os_sw_git_hash(&self) -> String {
        self.inner
            .info_get_os_sw_git_hash()
            .to_string_lossy()
            .into_owned()
    }
    ///  Flight software version type

    pub fn flight_sw_version_type(
        &self,
    ) -> crate::info::mavsdk::Info_Version_FlightSoftwareVersionType {
...
pub struct InfoClient {
    ...
    flight_information_tx: watch::Sender<std::option::Option<FlightInfoOwned>>,
}
...
impl InfoClient {
    ...
    pub fn subscribe_flight_information(
        &self,
    ) -> watch::Receiver<std::option::Option<FlightInfoOwned>> {
        let mut inner = self.inner.lock().unwrap();

        if inner.flight_information_handle.is_none() {
            // Lazy initialization of C++ 'flight_information' info stream.
            let user_data_ptr =
                Box::into_raw(Box::new(self.flight_information_tx.clone())) as *mut c_void;
...
impl Drop for InfoClient {
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(handle) = inner.flight_information_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_flight_information(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.flight_information_user_data
                            as *mut watch::Sender<std::option::Option<FlightInfoOwned>>,
                    );
                }
            }
        }
...
pub extern "C" fn info_flight_information_callback_ffi(
    ...
    let sender =
        unsafe { &*(user_data as *const watch::Sender<std::option::Option<FlightInfoOwned>>) };
