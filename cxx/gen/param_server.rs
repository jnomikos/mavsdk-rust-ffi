...
impl Drop for ParamServerClient {
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(handle) = inner.changed_param_int_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_changed_param_int(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.changed_param_int_user_data
                            as *mut watch::Sender<std::option::Option<IntParamOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.changed_param_float_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_changed_param_float(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.changed_param_float_user_data
                            as *mut watch::Sender<std::option::Option<FloatParamOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.changed_param_custom_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_changed_param_custom(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.changed_param_custom_user_data
                            as *mut watch::Sender<std::option::Option<CustomParamOwned>>,
                    );
                }
            }
        }
