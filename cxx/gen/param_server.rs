...
impl<'a> IntParam<'a> {
    ...
    pub fn name(&self) -> String {
        self.inner.param_server_get_name().to_string_lossy().into_owned()
    }
...
impl<'a> CustomParam<'a> {
    ...
    pub fn value(&self) -> String {
        self.inner.param_server_get_value().to_string_lossy().into_owned()
    }
}
...
pub struct AllParamsOwned {
        pub int_params: 
            std::vec::Vec<IntParamOwned>,
        pub float_params: 
            std::vec::Vec<FloatParamOwned>,
        pub custom_params: 
            std::vec::Vec<CustomParamOwned>,
}
...
impl<'a> AllParams<'a> {
    ...
    pub fn int_params(&self) -> 
            std::vec::Vec<IntParamOwned> {
            self.inner.param_server_get_int_params()
                .iter()
    ...
    pub fn float_params(&self) -> 
            std::vec::Vec<FloatParamOwned> {
            self.inner.param_server_get_float_params()
                .iter()
    ...
    pub fn custom_params(&self) -> 
            std::vec::Vec<CustomParamOwned> {
            self.inner.param_server_get_custom_params()
                .iter()
    ...
    impl ParamServerClient {
        ...
        pub fn subscribe_changed_param_int(&self) -> watch::Receiver<std::option::Option<
        IntParamOwned>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.changed_param_int_handle.is_none() {
                        // Lazy initialization of C++ 'changed_param_int' param_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.changed_param_int_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_changed_param_float(&self) -> watch::Receiver<std::option::Option<
        FloatParamOwned>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.changed_param_float_handle.is_none() {
                        // Lazy initialization of C++ 'changed_param_float' param_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.changed_param_float_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_changed_param_custom(&self) -> watch::Receiver<std::option::Option<
        CustomParamOwned>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.changed_param_custom_handle.is_none() {
                        // Lazy initialization of C++ 'changed_param_custom' param_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.changed_param_custom_tx.clone())) as *mut c_void;
