...
impl Drop for WinchClient {
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(handle) = inner.status_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_status(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.status_user_data
                            as *mut watch::Sender<std::option::Option<StatusOwned>>,
                    );
                }
            }
        }
