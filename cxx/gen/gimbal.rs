...
impl<'a> GimbalItem<'a> {
    ...
    pub fn vendor_name(&self) -> String {
        self.inner.gimbal_get_vendor_name().to_string_lossy().into_owned()
    }
        ///  Model name

    pub fn model_name(&self) -> String {
        self.inner.gimbal_get_model_name().to_string_lossy().into_owned()
    }
        ///  Custom name name

    pub fn custom_name(&self) -> String {
        self.inner.gimbal_get_custom_name().to_string_lossy().into_owned()
    }
...
impl<'a> ControlStatus<'a> {
    ...
    pub fn control_mode(&self) -> crate::gimbal::mavsdk::Gimbal_ControlMode {self.inner.gimbal_get_control_mode().clone()
    }
    ...
    impl GimbalClient {
        ...
        pub fn subscribe_gimbal_list(&self) -> watch::Receiver<std::option::Option<
        GimbalListOwned>> {
                ...
                }pub fn subscribe_control_status(&self) -> watch::Receiver<std::option::Option<
        ControlStatusOwned>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.control_status_handle.is_none() {
                        // Lazy initialization of C++ 'control_status' gimbal stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.control_status_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_attitude(&self) -> watch::Receiver<std::option::Option<
        AttitudeOwned>> {
                    let mut inner = self.inner.lock().unwrap();
