...
impl<'a> AdsbVehicle<'a> {
    ...
    pub fn altitude_type(&self) -> crate::transponder::mavsdk::Transponder_AdsbAltitudeType {
        self.inner.transponder_get_altitude_type().clone()
    }
    ...
    pub fn callsign(&self) -> String {
        self.inner
            .transponder_get_callsign()
            .to_string_lossy()
            .into_owned()
    }
    ///  ADSB emitter type.

    pub fn emitter_type(&self) -> crate::transponder::mavsdk::Transponder_AdsbEmitterType {
        self.inner.transponder_get_emitter_type().clone()
    }
    ///  Squawk code.
...
pub struct TransponderClient {
    ...
    transponder_tx: watch::Sender<std::option::Option<AdsbVehicleOwned>>,
}
...
impl TransponderClient {
    ...
    pub fn subscribe_transponder(&self) -> watch::Receiver<std::option::Option<AdsbVehicleOwned>> {
...
impl Drop for TransponderClient {
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(handle) = inner.transponder_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_transponder(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.transponder_user_data
                            as *mut watch::Sender<std::option::Option<AdsbVehicleOwned>>,
                    );
                }
            }
        }
...
pub extern "C" fn transponder_transponder_callback_ffi(
    ...
    let sender =
        unsafe { &*(user_data as *const watch::Sender<std::option::Option<AdsbVehicleOwned>>) };
