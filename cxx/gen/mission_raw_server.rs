...
impl Drop for MissionRawServerClient {
    fn drop(&mut self) {
        if let Ok(mut inner) = self.inner.lock() {
            if let Some(handle) = inner.incoming_mission_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_incoming_mission(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.incoming_mission_user_data
                            as *mut watch::Sender<std::option::Option<MissionPlanStatus>>,
                    );
                }
            }
            if let Some(handle) = inner.current_item_changed_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_current_item_changed(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.current_item_changed_user_data
                            as *mut watch::Sender<std::option::Option<MissionItemOwned>>,
                    );
                }
            }
            if let Some(handle) = inner.clear_all_handle {
                inner
                    .plugin
                    .pin_mut()
                    .unsubscribe_clear_all(handle as libc::uintptr_t);
                unsafe {
                    let _ = Box::from_raw(
                        inner.clear_all_user_data as *mut watch::Sender<std::option::Option<u32>>,
                    );
                }
            }
        }
