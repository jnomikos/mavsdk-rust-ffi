...
impl<'a> MissionPlan<'a> {
    ...
    pub fn mission_items(&self) -> 
            std::vec::Vec<MissionItemOwned> {
            self.inner.mission_raw_server_get_mission_items()
                .iter()
...
impl<'a> MissionProgress<'a> {
    pub fn new(inner: &'a crate::mission_raw_server::mavsdk::MissionRawServer_MissionProgress) -> Self {
        Self { inner }
    }
    ...
    pub struct MissionRawServerClient {
        inner: Mutex<MissionRawServerInner>,
                ...
                incoming_mission_tx: watch::Sender<std::option::Option<
            MissionPlanStatus>>,
                // Producer for the latest CurrentItemChanged state; broadcast to all active subscribers.
                current_item_changed_tx: watch::Sender<std::option::Option<
        MissionItemOwned>>,
                // Producer for the latest ClearAll state; broadcast to all active subscribers.
                clear_all_tx: watch::Sender<std::option::Option<
        u32>>,
    }

    impl MissionRawServerClient {
        pub fn new(plugin: cxx::UniquePtr<crate::mission_raw_server::mavsdk::MissionRawServer>) -> Self {
        ...
        pub fn subscribe_incoming_mission(&self) -> watch::Receiver<std::option::Option<
            MissionPlanStatus>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.incoming_mission_handle.is_none() {
                        // Lazy initialization of C++ 'incoming_mission' mission_raw_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.incoming_mission_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_current_item_changed(&self) -> watch::Receiver<std::option::Option<
        MissionItemOwned>> {
                    let mut inner = self.inner.lock().unwrap();

                    if inner.current_item_changed_handle.is_none() {
                        // Lazy initialization of C++ 'current_item_changed' mission_raw_server stream.
                        let user_data_ptr = Box::into_raw(Box::new(self.current_item_changed_tx.clone())) as *mut c_void;
                ...
                }pub fn subscribe_clear_all(&self) -> watch::Receiver<std::option::Option<
        u32>> {
                    let mut inner = self.inner.lock().unwrap();
